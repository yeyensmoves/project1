...
const _Client = class _Client {
  ...
  async chunkedUpload(method, url, headers = {}, originalPayload = {}, onProgress) {
    ...
    if (file instanceof inputFile.InputFile) {
      ...
      let response2 = null;
      while (start2 < size) {
        let end = start2 + _Client.CHUNK_SIZE;
        if (end >= size) {
          end = size;
        }
        headers["content-range"] = `bytes ${start2}-${end - 1}/${size}`;
        const chunk = await file.slice(start2, end);
        const payload = { ...originalPayload };
        payload[fileParam] = new nodeFetchNativeWithAgent.File([chunk], file.filename);
        response2 = await this.call(method, url, headers, payload);
        if (onProgress && typeof onProgress === "function") {
          onProgress({
            $id: response2.$id,
            progress: Math.round(end / size * 100),
            sizeUploaded: end,
            chunksTotal: Math.ceil(size / _Client.CHUNK_SIZE),
            chunksUploaded: Math.ceil(end / _Client.CHUNK_SIZE)
          });
        }
        if (response2 && response2.$id) {
          headers["x-appwrite-id"] = response2.$id;
        }
        start2 = end;
      }
      return response2;
