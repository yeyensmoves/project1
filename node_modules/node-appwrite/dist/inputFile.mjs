...
var InputFile = class _InputFile {
  ...
  static fromBuffer(parts, name) {
    if (parts && !ArrayBuffer.isView(parts) && typeof parts.arrayBuffer === "function") {
      return new _InputFile({ type: "blob", data: parts }, name);
    }
    if (typeof parts === "string") {
      return new _InputFile({ type: "buffer", data: new TextEncoder().encode(parts) }, name);
    }
    if (parts instanceof ArrayBuffer) {
      return new _InputFile({ type: "buffer", data: new Uint8Array(parts) }, name);
    }
  ...
  static fromPlainText(content, name) {
    return new _InputFile({ type: "buffer", data: new TextEncoder().encode(content) }, name);
  }
  async size() {
